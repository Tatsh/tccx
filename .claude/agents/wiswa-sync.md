...
`package.json` or `.pre-commit-config.yaml` edits, etc.), you
...
For example, if a script is added to `package.json`, the override in `.wiswa.jsonnet` is
...
2. **Read the changed managed files** (e.g. `package.json`, `.pre-commit-config.yaml`)
