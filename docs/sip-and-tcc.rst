...
- The TCC databases live under ``…/Application Support/com.apple.TCC/``, directories tagged
  ...
  :doc:`sip-filesystem-protection` §2).
- ``tccd`` carries the matching storage-class entitlements - both names are present in the
  binary:
...
**TCC integrity is a SIP guarantee.** Remove SIP and you can forge any privacy grant; keep
SIP and even root cannot.

Two blocks
----------

:doc:`tcc-internals` §11 names "two walls." With the SIP suite we can state each in its
exact mechanism:
...
.. list-table::
   ...
   * - **1. Cannot write the file**
     - SIP filesystem protection: ``TCC.db`` dir is ``TCC``-storage-class protected; the
...
:doc:`tcc-internals` §11). Wall 1's anchor is the storage-class entitlement pair above. Both
walls are SIP-rooted: one is SIP's filesystem layer, the other is SIP's runtime/identity
layer (:doc:`sip-runtime-protection`).
...
.. list-table::
   ...
   * - Route
     ...
     - SIP change needed
     - Verdict
   * - ``tcc-preapprove grant`` direct write (user DB)
     - yes
     - ``CSR_ALLOW_UNRESTRICTED_FS`` off (or ≤10.15 FDA hole)
     - Works only SIP-off on 11+
   * - Direct write (system DB)
     - yes
     - SIP-off **and root**
   ...
   * - Inject / task-port ``tccd``
     - n/a
     - ``CSR_ALLOW_TASK_FOR_PID`` off + defeat library validation
     - Impossible while SIP on
   * - **MDM PPPC profile**
     - **no** - ``tccd`` evaluates it *live* from ``MDMOverrides.plist``
     - **none**
     - The sanctioned, SIP-free path

The last row is the important one: the **PPPC/MDM** path (:doc:`tcc-internals` §13–15) never
writes the protected file, so it sidesteps SIP entirely - at the cost of requiring
user-approved MDM / supervision. It is the only route that is both unprivileged-friendly
*and* distributable.
