...
> Scope per [`README.md`](README.md): unmarked prose is established XNU/macOS internals
> (from `bsd/sys/csr.h` and the `csrutil`/`libsystem` interfaces). **[verified]** marks
> repo-grounded claims.

## 1. One word, eleven+ bits
...
- **Intel Macs:** the firmware NVRAM variable **`csr-active-config`** (a little-endian
  ...
  ```
  but you **cannot meaningfully *write* it while booted** — setting it is itself a
...
## 3. "Disabled" is not a magic number — decode the bits
...
## 4. Reading SIP state programmatically (the right way)
...
## 5. Changing SIP state (disable / enable)
...
2. Terminal → `csrutil disable` (or `csrutil enable`, or partial: `csrutil enable
   --without fs`, depending on version).
...
3. For *system-volume* edits you must additionally
...
Every "persist" operation this project's direct-DB tools attempt depends on **bit 1**
being clear. The mechanics above are the full surface a user must touch to make
`tcc-preapprove grant` actually stick, and the reason the repo's stance is "tools kept,
SIP-off caveat" rather than "tools that just work."
