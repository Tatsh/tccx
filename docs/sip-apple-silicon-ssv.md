...
> Scope per [`README.md`](README.md): this repo's `tccd` is the **Intel 10.15.6** slice,
> which predates Apple Silicon and SSV. This document is therefore **documented macOS
> internals** (Apple Platform Security guide + public research), not repo-verified. It
> exists because anyone re-running this project on modern hardware will land here, and
> because it explains why "SIP off" is necessary-but-no-longer-sufficient to persist
> system changes on 11+.

## 1. What changed after 10.15
...
still queried with `csr_check`/`csrutil`, but it now sits *inside* this larger structure.

## 2. The Signed System Volume
...
1. `csrutil authenticated-root disable` — clears **`CSR_ALLOW_UNAUTHENTICATED_ROOT`**
   (bit 11), telling the boot chain to accept an *unsealed* / custom-snapshot root;
...
## 3. LocalPolicy and security levels (Apple Silicon)
...
## 4. Detecting state on modern macOS
...
## 5. Bottom line for this project on 11+/Apple Silicon
...
- You generally do **not** need to break SSV for a TCC write (TCC is on the data volume),
  but you *do* on any task that touches `/System`.
