...
> `0x100000000`) in the Ghidra project `tcc` (ghidra-mcp port **8090**). All addresses,
> SQL, and enum values below were read directly out of this binary — not from memory or
> blog posts. Where a value differs from commonly-cited online tables, **this binary wins**
> for 10.15.6.
...
## 2. `access` table schema (verified)
...
| `csreq` | BLOB | serialized `SecRequirement` (magic `0xfade0c00`); see §5 |
...
## 3. Service constants (verified strings)
...
## 4. `auth_reason` enum (verified from `GetAuthReasonString` @ `0x100001e80`)
...
## 5. The `csreq` contract (from `-[TCCDAccessIdentity matchesCodeRequirementData:]` @ `0x10001f1c5`)
...
if (status == 0)                         // blob parsed
    ...
    return false;                        // → NO MATCH → row does not authorize
}
```

Consequences:
- `csreq` **fails closed**. A NULL or garbage blob makes the row useless — it does *not*
  default to "allow".
- The blob must be a real serialized `SecRequirement`, and the **running** client must
  satisfy it (designated requirement match). Generate it from the target binary's own DR.

## 6. Write-path provenance (who writes what)
...
## 7. Why there is no "private API" to inject a grant
...
`3`=limited, the DB write path tightened to SIP-off only, and the cache-reload behavior
changed. Re-verify `.schema access` and re-read `GetAuthReasonString` on the target OS.

## 11. The two walls (why direct writes need SIP off, and why there's no shortcut)
...
1. **SIP filesystem protection on `TCC.db`.** Both databases are SIP-protected paths.
   `tccd` can write them because it's an Apple-signed *platform binary* exempted on those
...
- `tccd` is a separate **process**, not a library. `dlsym` resolves symbols in *your*
...
- Running code *inside* `tccd`'s process needs its task port → blocked by SIP + hardened
...
- "Disable SIP → write → re-enable" *does* leave valid grants (csreq is validated against
...
## 13. PPPC `.mobileconfig` generation (the SIP-free, distributable path)
...
## 14. CLI reference (`tcc-preapprove` — SwiftPM package at repo root)
...
## 15. Deploying the PPPC profile (the UAMDM requirement)

Generating the `.mobileconfig` (§13) is the easy half. Getting macOS to **honor** it is
...
**only applied when the profile is delivered by MDM**, and that enrollment must be
...
11+** (the CLI can still `list`/`remove`). So on 11+ with no MDM there is *no* path to an
...
| **Self-hosted MDM** (`nanomdm` / `micromdm`) | medium | The only way to make PPPC work on *your own* machine without a commercial MDM. |
...
3. **Enrollment profile** — serve an enrollment `.mobileconfig` (MDM payload pointing at
   your server's check-in/command URLs). Install it on the Mac.
4. **User-approve the enrollment (UAMDM)** — on the Mac, approve Device Management in
   System Settings (Apple-silicon requires the local user to click Allow). Without this
   approval the enrollment is *not* user-approved and PPPC still won't apply.
...
- **Profile/MDM grants are NOT written to the `access` table.** `tccd` evaluates them
  *live* via `-[TCCDPolicyOverride evaluateOverridePolicyForAccessByIdentity:…]`
...
e.g. the unsigned-code policy *deny*: `auth_value = 0; auth_reason = 5` (logged
...
> Honest scope note: the override *evaluation* (sources, keys, csreq match, value
...
> initializes to `0`/None and is written to `5` **only** in that deny branch; the allow
...
> *deny* → reason 5 as verified; profile *allow* reason as **unresolved**. (The `nanomdm`
> enrollment sequence above is likewise the standard documented flow, not run end-to-end.)
