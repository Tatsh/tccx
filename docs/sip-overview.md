...
list of operations are denied _to every process regardless of uid_ - including uid 0 —
...
SIP raises the floor: even _if_ an attacker gets root, they still cannot
...
## SIP is a _family_ of checks, not a single gate
...
"SIP is off," always ask _which bits_ - `csrutil status` and `csr_check()` answer per
...
stores them in `TCC.db`. SIP is what makes those decisions _trustworthy_: the TCC
...
TCC's integrity _depends on_ SIP.
...
- **To persist a row by writing `TCC.db` directly, you must disable SIP's filesystem
  ...
  policy make this effectively "SIP fully off." This is _not_ a bug to route around - it
