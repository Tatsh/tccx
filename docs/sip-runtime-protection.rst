...
- Even a process that *is* allowed to debug others (with the bit set) still will not satisfy
...
This is the runtime half of "you cannot become ``tccd``."

Kernel extension signing - bits 0 and 9
---------------------------------------

``CSR_ALLOW_UNTRUSTED_KEXTS`` (bit 0) and ``CSR_ALLOW_UNAPPROVED_KEXTS`` (bit 9) control loading of
kernel code. With SIP on, only Apple-signed (and, on newer OSes, user-approved / notarized)
kexts load. This blocks the "drop a kext that bypasses TCC/SIP from ring 0" approach: you would
...
Platform binaries (and how ``tccd`` uses the concept)
------------------------------------------------------------------
...
is a platform binary. From this repo's ``tccd``:
...
sibling of SIP rather than a ``csr`` bit, but it is part of the same "you cannot load your code
